...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	for(int i=0;i<nboxes;i++){
	   printf("Box %d at (x,y)=(%f,%f) with (w,h)=(%f,%f) %d\n", i, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h, dets[i].sort_class); 
	   for(int j = 0; j < l.classes; ++j){
		   if (dets[i].prob[j] > thresh){
			    printf("\t%s %.0f%%\n",names[j],(dets[i].prob[j])*100+0.5f);
		   }
	   }
	}
...
#else
#ifdef OPENCV4
        	save_image_to_jpeg_file(im, "predictions");
#else
